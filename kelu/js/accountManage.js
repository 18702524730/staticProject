...
// to更新手机号
function toUpdateMobile(externalUrl, params, on_result) {
  $.ajax({
    url : externalUrl + "user/to-update-mobile.json",
    type : 'POST',
...
$(function() {
  ...
  var initInfo = function(data) {
  	...
  	if (data.type == 1) {
  	  ...
  	  if (data.head_portrait_url) {
  	    var photo_src = externalUrl + data.head_portrait_url + '&t='+ Math.random();
  	    $('.photo img', c).attr('src', photo_src);
  	    $('.login_name img').attr('src', photo_src);
  	  }
  ...
  if (modify_box.length) {
    ...
    $('.next', modify_box).click(function(e) {
      ...
      // 验证就旧手机号
      toUpdateMobile(externalUrl, ret, function(data) {
        if (data.code === 0) {
    ...
    $('.code_new', modify_box).click(function() {
      ...
      var mobileField = $('.mobile_new', modify_box);
      var mobile = mobileField.val();
      if (!mobile) {
        $.message('异常，手机号不存在', 'error');
        return;
  ...
  if (p_verifyEls.length) {
  	...
  	// 提交点击
  	var person = $("#person_verify");
  	$('.fir .submit', person).click(function(e) {
  	  var name = $('.name', person).val();
  	  var id_number = $('.id_number', person).val();
  	  var id_number_front = $('.id_number_front', person)[0].files[0];
  	  var id_number_verso = $('.id_number_verso', person)[0].files[0];
  	  var mobile = $('.mobile', person).text();
  	  var verify_code = $('.verify_code', person).val();
